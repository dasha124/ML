...
def load_data():
    ...
    Загрузка данных
    '''
    data = pd.read_csv('pock_1 (1).txt', sep=",", nrows=500)
    return data
...
def preprocess_data(data_in):
    ...
    scale_cols = ['total', 'sp_attack', 'sp_defense']
    new_cols = []
    sc1 = MinMaxScaler()
    ...
    return data_out[new_cols], data_out['legendary']
...
clf_gs = GridSearchCV(KNeighborsClassifier(p=p, weights=selected_weight, algorithm=selected_algorithm), tuned_parameters, cv=cv_slider, scoring='roc_auc')
clf_gs.fit(data_X, data_y)

st.subheader('Оценка качества модели')
st.write('Лучшее значение параметров - {}, Лучшая оценка - {}'.format(clf_gs.best_params_, clf_gs.best_score_))
